...
class EmailChannel(BaseChannel):
    ...
    def _validate_config(self) -> bool:
        ...
        if (
            self.config.verify_spf or self.config.verify_dkim
        ) and not self.config.trusted_authserv_ids:
    ...
    def _check_authentication_results(self, parsed_msg: Any, sender: str) -> tuple[bool, bool]:
        ...
        if (
            len(from_headers) != 1
            or any(
                not isinstance(defect, UndecodableBytesDefect) for defect in from_headers[0].defects
            )
    ...
    def _parse_authentication_results_header(
        ...
        if (
            len(parts) < 2
            or re.fullmatch(
                r'(?:"[a-zA-Z0-9][a-zA-Z0-9._:-]*"|[a-zA-Z0-9][a-zA-Z0-9._:-]*)(?:\s+1)?', parts[0]
            )
            is None
        ):
            return no_results
        ...
        while index < length:
            ...
            if value[index] == '"':
                ...
                if (
                    index < length
                    and value[index] == "@"
                    and key.casefold() in {"smtp.mailfrom", "header.i"}
                ):
